...
class Menu(Panel):
    ...
    def __init__(self, title: str, dimensions: tuple, align: Alignment = Alignment.LEFT,
                 items: list = []):
    ...
    def get_selected(self):
        """
        Comprehend a list of selected items and return the first if one
        selected, otherwise return None.

        Returns: First selected item, otherwise None if empty.
    ...
    def handle_events(self, key):
        if key == curses.KEY_UP:
            ...
            self.next()
        elif key == curses.KEY_ENTER or key == NEW_LINE \
...
if __name__ == '__main__':
    menu = Menu("app", (30, 30, 10, 10), align=Alignment.LEFT, items=["house", "ewak", "month"])
