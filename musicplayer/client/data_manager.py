...
class DataManager(object):
    ...
    def search_artist(self, criteria):
        ...
        if not items:
            raise EmptyResultError(f'Could not find the artist: {criteria}')
    ...
    def get_artist_albums(self, artist_id, max_items=20):
        ...
        if not albums:
            raise EmptyResultError(('Could not find any albums for'
                                    f'the artist_id: {artist_id}'))

        return [MenuItem(self._format_artist_label(album), album) for album in
                albums[:max_items]]
    ...
    def get_album_tracklist(self, album_id):
        ...
        if not results:
            raise EmptyResultError('Could not find the tracks for the album')
